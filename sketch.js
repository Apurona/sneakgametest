...
// **ゲーム状態**
const MOVE_INTERVAL = 750; // 移動間隔をミリ秒で定義 
...
// =======================================================
function preload() {
    // ファイル名が 'backgroundHebi.jpg' であると仮定します
...
function draw() {
    // gameState に応じて描画内容を切り替える
    if (gameState === 'title') {
        drawTitleScreen();
    } else {
        // ゲームプレイ中/ゲームオーバー時の背景を白に
        background(255); 
...
    // splice(開始インデックス, 削除する要素数)
    // newLength から最後までを削除します。
    // 例: 長さ10 -> newLength=5 の場合、snake.splice(5) で 5要素目以降が削除され、長さが 5 になります。
